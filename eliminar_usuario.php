...
if (!isset($_POST['usuario_id']) || empty($_POST['usuario_id'])) {
    header('Location: admin.php?error=No se especificó un usuario para eliminar');
    exit();
}
...
try {
    ...
    header('Location: admin.php?message=Usuario eliminado correctamente');
} catch (Exception $e) {
    // Revertir transacción en caso de error
    $pdo->rollBack();
    // Redirigir con mensaje de error detallado
    header('Location: admin.php?error=Error al eliminar usuario: ' . urlencode($e->getMessage()));
}
